...
func uploadToS3(svc *s3.S3, bucketName string, key string, data []byte) error {
...
func main() {
    ...
    sess, err := session.NewSession(&aws.Config{
        Region: aws.String("us-east-1"), // Set your AWS region
    })
    if err != nil {
    ...
    // Upload Companies to S3
    if err := uploadToS3(s3Client, bucketName, fmt.Sprintf("%s/companies/%s", databaseName, companyFilename), companyData.Bytes()); err != nil {
        log.Fatalf("Failed to upload company data to S3: %v", err)
    }
    ...
    if err := uploadToS3(s3Client, bucketName, fmt.Sprintf("%s/employees/%s", databaseName, employeeFilename), employeeData.Bytes()); err != nil {
        log.Fatalf("Failed to upload employee data to S3: %v", err)
    }
    ...
    // Upload Departments to S3
    if err := uploadToS3(s3Client, bucketName, fmt.Sprintf("%s/departments/%s", databaseName, departmentFilename), departmentData.Bytes()); err != nil {
        log.Fatalf("Failed to upload department data to S3: %v", err)
    }
